...
1. 文案已过期，以 `https://bbs.52pcgame.net/forum.php?mod=viewthread&tid=551078` 为准
2. 附录中存放论坛帖子源码
...
时光荏苒，距我第一次写修改笔记，已经过去十年了，钢4也出十周年了！

很多变了，也有很多一直没变，不管是钢4、修改笔记，亦或是我、大家！

钢4的机制已经趋于稳定，而当年的笔记还有很多不成熟、不兼容的地方，本次我就一并适配了。
...
- 模组通用化：将原版的省份修改、国家修改合并至一个通用开局事件，适配任意货国家开局
- 省份修改：新增资源煤、精简省份建筑类型、去除冗余修改（胜利点、地区建筑）
- 国家修改：新增开局科技（现代坦克、火箭引擎III）、去除冗余修改（凝聚力、执政政党、支持率、国家理念、海陆将领）
- 科技修改：精简全局增益、去除冗余修改（现代自行火炮、现代自行反坦克、现代自行防空）
- 坦克修改：新增现代坦克底盘修改、调整现代坦克兵种修改（适配衍生兵种）
- 飞机修改：新增火箭引擎修改，去除冗余修改（飞机类型、飞机任务、空投），仅适配小型飞机（战斗、支援、侦查）

MOD版附件下载，老规矩，回复可见。

如果各位看官觉得不错的话，欢迎版本加精，欢迎同学们打赏。

P.S. 前100名回帖随机 88PB，小小心意，不成敬意。
...
标签： 原创MOD，事件修改，省份修改, 国家修改, 科技修改, 全局增益修改，坦克修改, 飞机修改


## 二楼：MOD说明 

本次最大的调整是将模组通用化，支持任意国家开局，免除去每个国家下面修改的麻烦。

MOD使用官方启动器 `dowser.exe` 创建，空模组创建很简单，此处不再赘述。

模组内有详尽的注释，语法缩进也简洁明了，以便于大家按需调整。
...
MOD文件读取机制简要说明：
1. 同路径同名文件
    - MOD覆盖本体，如：`modern_armor.txt`、`tank_chassis.txt`
2. 同路径不同名，但是对象相同
    - 先按文件名排序(00_xx > zz_xx > ZZ_xx)，再决定读取机制
    - 以首次读取的为准，后续的跳过，如：`00_my_cheat_technologies.txt`
    - 以末次读取的为准，后续的完全覆盖前面的，如：`zz_my_cheat_modules.txt`
    - 依次读取同对象后，后续的合并覆盖至前面的，该情况较复杂，本次未用到
...
事件由两个部分组成（事件触发、事件内容），具体写法如下：


事件触发: `common/on_actions/my_cheat_actions.txt`
...
事件内容: `events/my_cheats_events.txt`

```shell
add_namespace = my_cheat_events
...
科技修改采用增量修改的方式，将本体中下述两个文件中的指定对象提取出来，合并放在一个文件中，再做少量修改即可，注意文件命名排序。

- 现代坦克科技: `Hearts of Iron IV\common\technologies\NSB_armor.txt` -> `main_battle_tank_chassis`
- 火箭引擎III科技: `Hearts of Iron IV\common\technologies\electronic_mechanical_engineering.txt` -> `sp_rockets_dual_chamber_rocket_engine_2`

关键内容给如下，修改位置均有详细注释，未变更处以`....`指代。
...
坦克修改采用全量的方式，将本体中下述两个文件中的拷贝一份，再做少量修改即可。
P.S. 使用增量模式时，由于存在部分参数未被更新，我放弃增量方案，采用全量模式，此为遗憾。
关键内容给如下，修改位置均有详细注释，未变更处以`....`指代。

现代坦克兵种：`Hearts of Iron IV\common\units\modern_armor.txt` -> `modern_armor`
...
现代坦克底盘：`Hearts of Iron IV\common\units\equipment\tank_chassis.txt` -> `modern_tank_chassis`
...
飞机修改采用增量的方式，将本体中下述文件中的指定对象提取出来，再做少量修改即可，注意文件命名排序。
关键内容给如下，修改位置均有详细注释，未变更处以`....`指代。

火箭引擎III：`Hearts of Iron IV\common\units\equipment\modules\00_plane_modules.txt` -> `rocket_engine_3`
...
先发个牢骚：十年过去了，论坛的编辑器依然一言难尽呀，幸亏有AI帮忙才勉强搞定。


友情提示：

1. 文本编辑器建议：VSCode （支持语法高亮 和 段落折叠）
2. 该模组我自己可以准完美运行（尚未出错）
3. 装甲部队无需配置支援连队
