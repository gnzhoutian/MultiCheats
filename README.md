...
3. 俘获流能触发碾压吗？ 如果能则可以保留弹药，如果不能则加硬攻，如果加硬攻，则不要用反坦克了， 对步兵太弱
4. 工兵上了装甲车时，是不能进行扫雷的
5. 侦察车比步兵强
6. 反坦克 + 软攻， 侦察车 + 硬攻，才能比较均衡，这种情况下没必要加弹药，也无法碾压，不过侦察车容易缴获，反坦克硬攻伤害高但移动低
7. 坦克的软硬攻其实是最均衡的，但是在使用中，碾压可能不生效，两种功能的坦克混在一起也不太好看，因而感觉还是侦察车机动高最划算
8. 对硬攻击感觉+8是不够的，并不能俘获后期的T34 25左右的防御力，感觉应该加到12差不多够
...
function CreateHeroCapture(name, portrait)
    ...
    hero.attack_modifiers = {
        -- {type = TargetType.Soft,    mod = 8},       -- 对软攻击 +8
        {type = TargetType.Hard,    mod = 12},       -- 对硬攻击 +12
    }
    hero.extra_traits = {
